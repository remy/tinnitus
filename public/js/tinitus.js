...
function createNotch(end) {
  var node = context.createBiquadFilter();
  node.type = node.NOTCH;
  node.frequency.value = centreFreq(el.freqSlider.value);
  node.Q.value = 1;
  node.connect(end);
...
function bandstop(stereo, bufSize){
  ...
  node.frequency.__defineGetter__('value', function () {
    return value;
  });
  node.frequency.__defineSetter__('value', function (v) {
    value = v;
    lower = value / 1.41;
    upper = lower * 2;
  });

  // Object.defineProperties( node.frequency, {
  //   value: {
  //     get: function() {
  //       return value;
  //     },
  ...
  node.onaudioprocess = function(e){
    ...
    for (var i = 0; i < bufSize; i++){
      ...
      outBufferR[i] = data;
      outBufferL[i] = data;
      // outBufferR[i] = false ? inBufferR[i] * 0.1 : inBufferL[i];
    }
  };
  return node;
}
...
function setupEventHandlers() {
  ...
  el.notch.onclick = function () {
    ...
    notch = createNotch(analyser);
    window.n1 = createNotch(notch);
    window.n2 = createNotch(window.n1);

    notch.frequency._value = notch.frequency.value;
    notch.frequency.__defineGetter__('value', function () {
      return this._value;
    });
    notch.frequency.__defineSetter__('value', function (v) {
      this._value = v;
      n1.frequency.value = v;
      n2.frequency.value = v;
    });

    /*Object.defineProperties(notch.frequency, {
      value: {
        get: function() {
          return this._value;
        },
    ...
    // noise -> n2 -> n1 -> notch -> analyser

    if (noise) {
      noise.disconnect();
      noise.connect(window.n2);
  ...
  el.freqSlider.onchange = function () {
    ...
    var note = noteFromPitch(this.value);
    var cent = centsOffFromPitch(this.value, note);
...
function update() {
  ...
  gfx.beginPath();
  gfx.lineCap = 'round';
  gfx.fillStyle = 'hsla(180, 100%, 25%, 0.5)';
  gfx.strokeStyle = '#900';
  gfx.lineWidth = 2;
